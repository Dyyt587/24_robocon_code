...
int FSUS_Init(void)
{
	...
	struct serial_configure config = {
		...
		0};
	if (RT_EOK != rt_device_control(servo1.uart, RT_DEVICE_CTRL_CONFIG, &config))
	{
		rt_kprintf("change %s(servo) failed!\n", servo1.uart->parent.name);
	}
...
FSUS_STATUS FSUS_WheelMove(servo_t *servo, uint8_t servo_id, uint8_t method, uint16_t speed, uint16_t value)
{
	...
	if (rt_mutex_take(servo->mutex, FSUS_TIMEOUT_MS) != RT_EOK)
	{
		LOG_E("fsus wheel mode take mutex timeout");
		return FSUS_STATUS_TIMEOUT;
	}
	// 发送请求包
