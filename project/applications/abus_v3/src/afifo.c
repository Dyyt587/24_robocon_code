  ...
  unsigned int afifo_in(struct afifo* fifo, const unsigned char* buffer, unsigned int len)
{
 ...
 inline unsigned int afifo_out_data(struct afifo* fifo, unsigned char* buffer, unsigned int len)
{
    unsigned long flags;
    unsigned int ret;
    spin_lock_irqrestore(fifo->lock, flags);
 ...
 inline unsigned int afifo_add_out(struct afifo* fifo, unsigned int len)
{
    unsigned int ret;
    len = min(len, fifo->in - fifo->out);
    fifo->out += len; // 每次累加，到达最大值后溢出，自动转为0
 ...
 * @return unsigned int
 */
 inline unsigned int afifo_get_free_block(struct afifo* fifo)
{
